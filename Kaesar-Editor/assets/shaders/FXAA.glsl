...
void main()
{
    ...
    v_TexCoords.xy = a_TexCoords * vec2(pc.width, pc.height);

    vec2 rcpFrame = vec2(1 / (pc.width), 1 / (pc.height));
	v_TexCoords.wz = a_TexCoords - (rcpFrame * (0.5));
}
...
void main()
{
    ...
    vec2 v_rgbNW = (v_TexCoords.xy + vec2(-1.0, -1.0)) * inverseVP;
	vec2 v_rgbNE = (v_TexCoords.xy + vec2(1.0, -1.0)) * inverseVP;
	vec2 v_rgbSW = (v_TexCoords.xy + vec2(-1.0, 1.0)) * inverseVP;
	vec2 v_rgbSE = (v_TexCoords.xy + vec2(1.0, 1.0)) * inverseVP;
	vec2 v_rgbM  = vec2(v_TexCoords.xy * inverseVP);
    ...
    vec3 rgbA = 0.5 * (
        ...
        texture(u_Texture, v_TexCoords.xy * inverseVP + dir * (2.0 / 3.0 - 0.5)).xyz);
    vec3 rgbB = rgbA * 0.5 + 0.25 * (
        texture(u_Texture, v_TexCoords.xy * inverseVP + dir * -0.5).xyz +
        texture(u_Texture, v_TexCoords.xy * inverseVP + dir * 0.5).xyz);
